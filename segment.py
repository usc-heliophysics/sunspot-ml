...
def clear_bg(clustered_img, bwidth=10, bg_min_count=50, return_vals=False):
    ...
    unique_vals, counts = cp.unique(border, return_counts=True)
    unique_vals = unique_vals[cp.where(counts > bg_min_count)]
    R[cp.argwhere(cp.in1d(R, unique_vals))] = 0
...
def binarize_features(cleared_img, feature='penumbrae'):
    ...
    if nonzero_uniques.any():
        binarized = cp.copy(cleared_img)
        if feature == 'penumbrae':
            binarized[binarized > 0] = 1
        elif feature == 'umbrae':
            binarized[binarized > cp.min(nonzero_uniques)] = 0
            binarized[binarized == cp.min(nonzero_uniques)] = 1
        else:
...
def segment_core(fits_path, image_path=None, feature="penumbrae", findroi_kwargs=None, kmeans_kwargs=None, clearbg_kwargs=None):
    ...
    with open(regionprop_path, 'w') as file:
        ...
        for region in props:
            ...
            min_intensity = cp.min(region_slice)
            min_intensity_index = cp.argmin(region_slice)
            min_intensity_coords = cp.unravel_index(min_intensity_index, region_slice.shape)
            ...
            mean_intensity = int(cp.mean(region_slice))
